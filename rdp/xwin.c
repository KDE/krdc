...
BOOL
ui_init(void *_display, void *_visual)
{
	XPixmapFormatValues *pfm;
	uint16 test;
	int i;

	display = _display ? _display : XOpenDisplay(NULL);
	if (display == NULL)
	{
	...
	visual = _visual ? _visual : DefaultVisualOfScreen(screen);
...
ui_create_window(int _wnd)
{
	...
	wnd = _wnd > -1 ? _wnd : XCreateWindow(display, RootWindowOfScreen(screen), 0, 0, wndwidth, wndheight,
